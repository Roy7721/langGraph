...
# The Windows console defaults to cp1252, which cannot encode characters the model
# happily returns (non-breaking hyphens, curly quotes, the degree sign).
# Printing them raises UnicodeEncodeError, so force UTF-8 on stdout.
...
# Fix 1: Gives background uploads more time. Without this the script also stalls on
# exit, because the atexit flush retries the trace upload against a 3s connect budget.
...
# gpt-oss-120b is a native tool-calling model, which is exactly what
# with_structured_output needs - it binds the schema as a tool under the hood.
# (This is the same trait that made it unusable for the text-ReAct agent in 4_agent.py.)
